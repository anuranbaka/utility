// THIS VERSION DEPENDS ON GPL CODE! IT IS THUS NOT PUBLIC DOMAIN!!
...
#include "stack_dump_on_assert_bfd.h"
...
static inline void printStackTrace( FILE *out, unsigned int max_frames)
{
    ...
    // this array must be free()-ed
    char** symbollist = backtrace_symbols( addrlist, addrlen );
    char** filelinelist = backtraceSymbols( addrlist, addrlen ); //take the line info from here
   ...
   for ( unsigned int i = 4; i < addrlen; i++ )
   {
      ...
      if ( begin_name && end_offset && ( begin_name < end_offset ))
      {
         ...
         *end_offset++   = '\0';
         ...
         if ( begin_offset )
         {
            fprintf( out, "%40s @ %-30s  %-30s ( %-40s  + %-6s ) %s\n",
                     fname, filelinelist[i], symbollist[i], fname, begin_offset, end_offset );
         } else {
            fprintf( out, "%40s @ %-30s  %-30s ( %-40s    %-6s ) %s\n",
                     fname, filelinelist[i], symbollist[i], fname, "", end_offset );
         }
